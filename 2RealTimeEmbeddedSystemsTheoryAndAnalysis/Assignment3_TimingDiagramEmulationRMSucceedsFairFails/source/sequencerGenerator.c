...
void *Service_1(void *threadp)
{
    ...
    while(!abortService1)
    {
        ...
        if(!abortService2)
        {
          ...
          syslog(LOG_INFO, "[COURSE:2][ASSIGNMENT:3]: Thread 1 start %i @ <%f> on core <%i>", serviceCounter, timeMs, core);
          closelog();
        }
    }
    pthread_exit((void *)0);
...
void *Service_2(void *threadp)
{
    ...
    while(!abortService2)
    {
        ...
        if(!abortService2)
        {
          ...
          openlog("pthread", LOG_PID|LOG_CONS, LOG_USER);
          syslog(LOG_INFO, "[COURSE:2][ASSIGNMENT:3]: Thread 2 start %i @ <%f> on core <%i>", serviceCounter, timeMs, core);
          closelog();
         }
    }
    pthread_exit((void *)0);
...
void *Service_3(void *threadp)
{
    ...
    while(!abortService3)
    {
        ...
        if(!abortService2)
        {
          ...
          openlog("pthread", LOG_PID|LOG_CONS, LOG_USER);
          syslog(LOG_INFO, "[COURSE:2][ASSIGNMENT:3]: Thread 3 start %i @ <%f> on core <%i>", serviceCounter, timeMs, core);
          closelog();
        }
    }
    pthread_exit((void *)0);
...
void *Service_4(void *threadp)
{
    ...
    while(!abortService4)
    {
        ...
        if(!abortService2)
        {
          ...
          openlog("pthread", LOG_PID|LOG_CONS, LOG_USER);
          syslog(LOG_INFO, "[COURSE:2][ASSIGNMENT:3]: Thread 4 start %i @ <%f> on core <%i>", serviceCounter, timeMs, core);
          closelog();
        }
    }
    pthread_exit((void *)0);
...
int main (int argc, char *argv[])
{
    ...
    openlog("pthread", LOG_PID|LOG_CONS, LOG_USER);
    syslog(LOG_INFO, "[COURSE:2][ASSIGNMENT:3]: %s", buffer);
    closelog();    
    ...
    pthread_create(&threads[3], &attr, Service_4, (void *)&threadParams[0]);
